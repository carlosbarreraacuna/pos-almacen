...
class StoreStockAdjustmentRequest extends FormRequest
{
    ...
    public function messages(): array
    {
        return [
            ...
            'items.required' => 'Debe incluir al menos un producto en el ajuste.',
            'items.min' => 'Debe incluir al menos un producto en el ajuste.',
            'items.*.product_id.required' => 'El producto es obligatorio.',
            'items.*.product_id.exists' => 'El producto seleccionado no existe.',
            'items.*.current_quantity.required' => 'La cantidad actual es obligatoria.',
            'items.*.current_quantity.min' => 'La cantidad actual no puede ser negativa.',
            'items.*.adjusted_quantity.required' => 'La cantidad ajustada es obligatoria.',
    ...
    {
        // Si no se especifica fecha de ajuste, usar hoy
        if (!$this->has('adjustment_date')) {
            $this->merge(['adjustment_date' => now()->format('Y-m-d')]);
        }

        // Calcular quantity_adjustment para cada item
        if ($this->has('items')) {
            $items = $this->items;
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            if ($this->location_id && $this->warehouse_id) {
                $location = \App\Models\Location::find($this->location_id);
                if ($location && $location->warehouse_id !== (int)$this->warehouse_id) {
            ...
            if ($this->items && $this->warehouse_id) {
            ...
            // Validar que haya al menos un cambio significativo
            if ($this->items) {
